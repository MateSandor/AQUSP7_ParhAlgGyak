...
void game_over( void );
void check_unchoosen_amount(int *ptr);
...
void difficulty( void )                                     //Function for choosing the difficulty level
{
    ...
    while( (diff != 1) && (diff != 2) )
    {
        printf("\t\tChoose a difficulty level 1 for beginner mode or 2 for a custom game:");
        scanf("%f", &diff);
        FLUSH;
        if( (diff != 1) && (diff != 2) )
        {
...
void guess( void )
{
    ...
    if( blank_minefield[x][y] != '-' )  // Already used
    {
    ...
    {
        blank_minefield[x][y] = minefield[x][y]; //Blanking chosen point
        if( minefield[x][y] == 0 )
        {
        ...
        check_unchoosen_amount(&blank_minefield[0][0]);
        guess();
    }
}
    ...
    for (i = 0; i < (N*M); i++)
        {
            ...
            if(*(ptr + i) == '-')
                {
                    unknown++;
                    printf("ASD:%d",unknown);
                }
        }
    if(unknown==total_mines)
        {
            win();
        }
    else
